...
#### 프라이버시 기반 (F4.0 완료 — AI 연동 전 필수)
- [x] `ConsentRecordEntity` + `ConsentManager` — Room v5, 정책 버전 추적 (DataStore boolean 금지)
- [x] `ConsentDialog` UI 구현 — 전송 데이터·제외 데이터 목록 명시 (07_claude_api.md 7.0절)
- [x] `PromptSanitizer` 구현 — 제목 50자 제한, 메모·첨부·NPC 이름 제외
- [x] `ConsentManager.canCallApi()` 동의+활성화 이중 확인 (isGranted + claudeApiEnabled)
- [x] 단위 테스트: `ConsentManager` 게이트 단위 — 동의 없으면 `canCallApi()=false` 확인 (MockK)
- [ ] 통합 테스트: `ClaudeRepository.send()` 호출 0회 확인 (`verify(exactly=0)`) — ClaudeRepository 구현 후 추가 예정
