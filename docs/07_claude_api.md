...
object AppSettingsDefaults {
    ...
    // ⛔ AI_CONSENT_GIVEN boolean 플래그 추가 금지 — ConsentRecordDao 단일 진실 공급원 위반
}

// ClaudeRepository: ConsentManager 단일 게이트 — DataStore boolean 동의 플래그 사용 금지
suspend fun canCallApi(): Boolean {
    return consentManager.canCallApi()       // isGranted(AI_OUTBOUND) && claudeApiEnabled
...
@Test
fun `AI 동의 없으면 API 호출 없이 폴백 반환`() = runTest {
    // ConsentManager.isGranted(AI_OUTBOUND) = false 시뮬레이션
...
fun `claudeApiEnabled=false 이면 API 호출 없이 폴백 반환`() = runTest {
    // isGranted(AI_OUTBOUND)=true 이지만 운영 토글 off
    coEvery { consentManager.canCallApi() } returns false
    val result = repo.generateCombatNarrative(character, task, combatResult)
    verify(exactly = 0) { apiService.generateMessage(any(), any()) }
}
...
Phase 5 Week 10 시작 전:
  ✓ ConsentRecordEntity + ConsentManager 구현 완료 (F4.0, v5)
  ✓ ConsentDialog → ConsentRecordDao 저장 완료 (DataStore boolean 금지)
